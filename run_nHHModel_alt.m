...
for segment_ind=1:length(sim_time_segment_seq)
    ...
    [~, spike_time_inds_cell] = cellfun(@(v) findpeaks(v, 'MinPeakHeight', -20, 'MinPeakProminence', 30), voltage_all_pop_snapshot_cell, 'UniformOutput', false);
    ...
    for i=1:size(voltage_all_pop_snapshot, 3)
        plot(voltage_all_pop_snapshot(:, 4, 1));
    end
    hold off;
    %}
end
...
figure(1);
hold on;
plot_neuron_pops = [2, 1, 4, 5, 7, 6];
for i=1:length(plot_neuron_pops)
    plot_population = plot_neuron_pops(i);
    plot_neuron_ind = 1;
    subplot(length(plot_neuron_pops), 1, i);
    spike_times = horzcat(spike_times_all_neuron_history{:,plot_population,plot_neuron_ind});
    spike_times = uint32(spike_times);
    spike_train_binary = false(T+dt, 1);
    spike_train_binary(spike_times) = true;
    plot([1:T+dt;1:T+dt], [spike_train_binary';zeros(size(1:T+dt))], 'k-');
    axis([0, T+dt+1, -1, 2]);
    set(gca,'TickDir','out') % draw the tick marks on the outside
    set(gca,'YTick', []) ;
    set(gca,'YColor',get(gcf,'Color'));
    title(strrep(neuron_pops(plot_population).name, '_', '-'));
end
hold off;
